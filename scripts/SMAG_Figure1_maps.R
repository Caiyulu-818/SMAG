...
mag.all <- read.csv("/Users/caiyulu/Desktop/MAGcode/MAG/drep/mag40039_50_10.csv")
sample.info <- read.csv("/Users/caiyulu/Desktop/MAGcode/paper/data_submit/submit/Supplemental information/Supplemental_tables/sample_Supplementary_TableS1.csv")

sample.info$ecosystem <- str_replace(sample.info$ecosystem, 
                                         pattern = "Agricultural Land", 
                                         replacement = "Cultivated land") 
...
arti<-filter(sample.info,ecosystem=="Artificial Surfaces")
...
places_robin_df <- places_robin_df %>% 
        mutate(ecosystem = sample.info$ecosystem,
               sample = sample.info$sample_name,
               freq = sample.info$mag_num) %>% 
        arrange(sample)
...
gp.map
ggsave("/Users/caiyulu/Desktop/MAGcode/paper/Figure/fig1_map.pdf",gp.map,height = 6,width = 7)
